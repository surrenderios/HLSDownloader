...
@interface DownloadingTableViewCell()<HLSDownloadItemDelegate>
...
- (void)setItem:(HLSDownloadItem *)item
{
    ...
    /*
    [self.posterImageView sd_setImageWithURL:[NSURL URLWithString:item.thumbImageUrl]
                            placeholderImage:nil];
    NSInteger index = [item.episode integerValue] + 1;
    self.nameLabel.text = [NSString stringWithFormat:@"%@ %ld",item.video_Name,index];
     */
...
- (void)setDownloadStatus:(HLSDownloadItemStatus)status {
    ...
    switch (status) {
        case HLSDownloadItemStatusWaiting:
            ...
            self.speedOrStateLabel.text = @"Cache_Waiting";
        ...
        case HLSDownloadItemStatusPaused:
            ...
            self.speedOrStateLabel.text = (@"Cache_Paused");
            self.speedOrStateLabel.textColor = kProgressNormalColor();
        ...
        case HLSDownloadItemStatusFailed:
            ...
            self.speedOrStateLabel.text = (@"Cache_Failed");
            self.speedOrStateLabel.textColor = kProgressNormalColor();
...
{
    self.speedOrStateLabel.text = speed;
}
